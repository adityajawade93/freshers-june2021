...
var listPassengers = require("./passengers.json");
var app = new koa();
...
router.get("/v1/passengers", function (ctx) {
    ...
    var totalPages = Math.round(listPassengers.length / size) + 1;
    if (typeof page === "number" && typeof size === "number" && page >= 0) {
        var first = page * size;
        var last = Math.min((page + 1) * size, listPassengers.length);
        var req_data = listPassengers.slice(first, last);
        var data = {
