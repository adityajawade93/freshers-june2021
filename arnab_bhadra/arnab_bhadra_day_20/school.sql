...
FROM student s JOIN studentclass sc ON s.sid = sc.sid AND cid ="1";

SELECT DISTINCT s.sid, s.sname,s.rollno,s.address
FROM student s JOIN studentclass sc ON s.sid = sc.sid JOIN classschedule cs ON sc.cid = cs.cid WHERE cs.ssid = '1';
