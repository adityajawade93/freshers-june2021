...
describe('get route validations',()=>{
    test('invalid page status',async()=>{
        ...
        const response = await request(server).get(`/v1/passangers?page=${page}&size=${size}`);
    ...
    test('valid page', async()=>{
        ...
        //console.log(passangerData.length);
        //expect(response.body.total).toBe(passangerData.legth);
...
describe('post route validation', ()=>{
    test('invalid request',async()=>{
        ...
        const response =await request(server).post('/v1/passangers').send(inputdata);
