...
class Passanger {
    constructor(_id, name, trips, airline, __v) {
        this._id = _id;
        this.name = name;
        this.trips = trips;
...
;
let passangerData = require('../passangers.json');
function isValidPassangerData(page, size) {
    let totalData = passangerData.length;
...
function fetchpassangerData(page, size) {
    ...
    let endIndex = Math.min(page * size + size - 1, totalData - 1);
...
function getPassangerById(id) {
    ...
    for (let i = 0; i < passangerData.length; i++) {
...
function fetchPassanger(ctx) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const passangerlist = fetchpassangerData(page, size);
...
function addPassanger(ctx) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const id = uuid();
            let AddingPassangerData = new Passanger(id, newPassangerData.name.trim(), newPassangerData.trips, newPassangerData.airline, newPassangerData.__v);
...
function updatePassanger(ctx) {
    let id = getPassangerById(ctx.params.id);
    if (id == -1) {
