...
function validateData(data: TodoSchema): boolean {
    if (
...
function goodResponse(ctx: any, type: string, message: Task | string | Record<string, Task>) {
    ctx.response.status = 200;
    ctx.response.type = type;
    ctx.body = message;
}

function badResponse(ctx: any, type: string, message: string) {
