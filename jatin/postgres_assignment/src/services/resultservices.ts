...
async function addMarks(studentID:string, subjectID:string, marks:number){
    return new Promise((resolve,reject)=>{
        client.query('INSERT INTO school.result values ($1,$2,$3)',[studentID, subjectID, marks],(err: any,res: any)=>{
            if(err)reject(err);
...
async function updateMarks(studentID:string, subjectID:string, marks:number){
    return new Promise((resolve,reject)=>{
        client.query('UPDATE school.result set marks = $3 where studentid = $1 and subjectid = $2',[studentID, subjectID, marks],(err:any,res:any)=>{
            if(err)reject(err);
            else resolve('query fetched');
        })
    })
...
async function getToppersMarks(classid: any, toplimit: any){
    ...
    let query = 
    `select s.studentid, s.name, sum(result.marks) as total_marks
